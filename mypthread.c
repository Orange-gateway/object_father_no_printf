	...
	else
	{
		...
		while(while_root != NULL)
		{
			if(strcmp(while_root->string,"api")==0)
			{
				...
				else if( strcmp("gw_up_child_mac",api->valuestring) == 0 )
				{
					cJSON *son_ret = cJSON_GetObjectItem(root,"retcode");
					if(son_ret->valueint == 0)
					{
						cJSON *son_gateway_data = cJSON_GetObjectItem(root,"data");
						cJSON *son_gateway_new_mac = cJSON_GetObjectItem(son_gateway_data,"mac_new");
						NET_F *p = NULL;
						p = net_head;
						while( p )
						{
							if( p->flag && !strcmp(son_gateway_new_mac->valuestring,p->mac))
							{
								cJSON *son_gateway_channel = cJSON_GetObjectItem(son_gateway_data,"channel");
								cJSON *son_gateway_pan_id = cJSON_GetObjectItem(son_gateway_data,"pan_id");
								uint8_t final_cmd[18] = {0x5a,0xa5,0x12,0x34,0x56,0x78,0x12,0x34,0x56,0x78,0x25,0x00,0x00,0x00,0x03,0x00,0x00,0x00};
								str_to_hex(final_cmd+15,son_gateway_channel->valuestring,1);
								str_to_hex(final_cmd+16,son_gateway_pan_id->valuestring,2);
								if(send_to_son_gw_have(final_cmd,18,p->fd_net) == -1)
									p->flag = 0;
								break;
							}
							p = p->next;
						}
						cJSON *son_gateway_old_mac = cJSON_GetObjectItem(son_gateway_data,"mac_old");
						replace_son_bind_mac(son_gateway_new_mac->valuestring,son_gateway_old_mac->valuestring);
					}
				}
				break;
			}
...
void pthread_v_send(uint8_t *u_data)
{
	...
	else if(u_data[10] == 0xf5 && NET_FLAG)
	{
		char *son_gateway_mac = (char *)malloc(13);
		memset(son_gateway_mac,0,13);
		memcpy(son_gateway_mac,u_data_str+4,12);
		son_gateway_mac[12] = '\0';
		char *son_pan_id = (char *)malloc(5);
		memset(son_pan_id,0,5);
		memcpy(son_pan_id,u_data_str+32,4);
		son_pan_id[4] = '\0';
		char *son_channel = (char *)malloc(3);
		memset(son_channel,0,3);
		memcpy(son_channel,u_data_str+30,2);
		son_channel[2] = '\0';
		cJSON *son_gateway_root = cJSON_CreateObject();
		cJSON_AddStringToObject(son_gateway_root,"co","orange");
		cJSON_AddStringToObject(son_gateway_root,"api","gw_up_child_mac");
		cJSON *son_gateway_data = cJSON_CreateObject();
		cJSON_AddStringToObject(son_gateway_data,"mac",son_gateway_mac);
		cJSON_AddStringToObject(son_gateway_data,"pan_id",son_pan_id);
		cJSON_AddStringToObject(son_gateway_data,"channel",son_channel);
		cJSON_AddItemToObject(son_gateway_root,"data",son_gateway_data);
		char *send_char = cJSON_PrintUnformatted(son_gateway_root);
		...
		free(son_gateway_mac);
		son_gateway_mac = NULL;
		free(son_pan_id);
		son_pan_id = NULL;
		free(son_channel);
		son_channel = NULL;
		cJSON_Delete(son_gateway_root);
		son_gateway_root = NULL;
	}
	free (u_data_str);
	u_data_str=NULL;
	free(u_data);
